...
class EventoDAOImpl implements EventoDAO {
  ...
  ///Flag per restituire dati mockati
  ...
  Future<bool> existById(int? id) async {
    if (isTestMode) {
      return Future.value(true); // O false, a seconda dei tuoi test case
    } else {
      try {
        Connection connection = await connector.openConnection();
        var result = await connection.execute(
  ...
  Future<List<EventoDTO>> findAll() async {
    if (isTestMode) {
      return Future.value([EventoDTO(
        id: 1,
        id_ca: 123,
        immagine: 'path/to/mock/image1.jpg',
        nomeEvento: 'Evento Mock 1',
        descrizione: 'Descrizione Mock 1',
        date: DateTime.now(),
        approvato: true,
        email: 'mock1@example.com',
        via: 'Via Mock 1',
        citta: 'Città Mock',
        provincia: 'Provincia Mock',
      )
      ]);
    } else {
      try {
  ...
  Future<EventoDTO?> findById(int id) async {
    if (isTestMode) {
      return Future.value(EventoDTO(
        id: id,
        id_ca: 123,
        immagine: 'path/to/mock/image.jpg',
        nomeEvento: 'Evento Mock',
  ...
  Future<bool> removeEventiScaduti() async {
    if (isTestMode) {
      ...
      try {
        ...
        var result = await connection.execute(
          Sql.named(
              'DELETE FROM public."Evento" WHERE data < CURRENT_TIMESTAMP'),
        );
        if (result.affectedRows >= 0) {
          return true;
        }
